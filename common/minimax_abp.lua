...
function miniMaxMove.chooseBestMove(boardState)
	...
	for i=1, #availableMoves, 1 do
		...
		local localBoardState = table.deepcopy(boardState)
...
function miniMaxMove.miniMaxMovesAlphaBetaPruning(boardState, playerSymbol, alpha, beta)
	...
	for i=1, #availableMoves, 1 do
		...
		if nextTurn == playerSymbol then 
			...
			local bestMaxValue = table.get_min_max({ -1000,value })[2]
			alpha = table.get_min_max({ alpha, bestMaxValue })[2]
			...
			local bestMinvalue = table.get_min_max({1000,value})[1]
			beta = table.get_min_max({ beta, bestMinvalue })[1]
